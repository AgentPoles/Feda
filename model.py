...
def _get_training_callbacks_global(model_name):
    global_model_name = f"{model_name}_global_model.h5"
    model_path = os.path.join("user_global_models", global_model_name)
    checkpoint = ModelCheckpoint(model_path, monitor='val_loss', save_best_only=True, mode='min', verbose=1)
    early_stopping = EarlyStopping(monitor='val_loss', patience=20, verbose=1, restore_best_weights=True)
    # tensorboard = TensorBoard(log_dir='./logs', histogram_freq=1, write_graph=True, write_images=True)
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=3, min_lr=1e-6, verbose=1)
...
def predict_top_n_words_local_global_unique_words(model1, model2, local_tokenizer, global_tokenizer, text, n1=3, n2=2):
    # Prepare the token list
    token_list1 = local_tokenizer.texts_to_sequences([text])[0]
    token_list1 = pad_sequences([token_list1], maxlen=SEQUENCE_LEN-1, padding='pre')


    token_list2 = global_tokenizer.texts_to_sequences([text])[0]
    token_list2 = pad_sequences([token_list2], maxlen=SEQUENCE_LEN-1, padding='pre')

    # Predict the next words with both models
    prediction1 = model1.predict(token_list1)
    prediction2 = model2.predict(token_list2)

    # Get the top n word indices from the first prediction
    top_indices1 = prediction1[0].argsort()[-n1:][::-1]
    # Translate indices to words
    predicted_words1 = [local_tokenizer.index_word[idx] for idx in top_indices1]
    ...
    for idx in extended_top_indices2:
        word = global_tokenizer.index_word[idx]
...
def train_local_model(training_text, validation_text, tokenizer, model_name, model=None, learning_rate=0.005):
    ...
    save_locally_trained_tokenizer(tokenizer,model_name)
    return model, tokenizer, X_val, y_val



def train_global_model(training_text, validation_text, tokenizer, model_name, model=None, learning_rate=0.005):
    # Update the tokenizer based on the training text
    tokenizer.fit_on_texts([training_text])
    ...
    # Define your callbacks, optimizer, and compile the model as before
    callbacks = _get_training_callbacks_global(model_name)
    new_optimizer = Adam(learning_rate=learning_rate)
    model.compile(optimizer=new_optimizer, loss='categorical_crossentropy', metrics=['accuracy'])
    ...
    save_globally_trained_tokenizer(tokenizer,model_name)
    return model, tokenizer, X_val, y_val


def save_globally_trained_tokenizer(tokenizer, model_name):
    user_tokenizer_name = f"{model_name}_global_tokenizer.pickle"
    tokenizer_path = os.path.join("global_tokenizers", user_tokenizer_name)
    with open(tokenizer_path, 'wb') as handle:
        pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)


def save_locally_trained_tokenizer(tokenizer, model_name):
    user_tokenizer_name = f"{model_name}_local_tokenizer.pickle"
    tokenizer_path = os.path.join("local_tokenizers", user_tokenizer_name)
    with open(tokenizer_path, 'wb') as handle:
        pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)


def evaluate_global_model(model, text, tokenizer, model_name):
    """
    Evaluate the trained model with test data.

    :param model: Trained model to evaluate.
    :param text: Raw text data for evaluation.
    :param tokenizer: Tokenizer used for the model.
    :param sequence_length: Maximum length of the sequences.
    :return: Loss, accuracy, and length of test data.
    """
    ...
    x, y = prepare_data(sequences, tokenizer)# Adjust num_classes if needed

    # Evaluate the model
    loss, accuracy = model.evaluate(x, y, verbose=0)   # Set verbose to 1 if you want to see the progress
    print("done")
    global_model_name = f"{model_name}_global_model.h5"
    save_model_to_file(model,global_model_name,model_dir="user_global_models")
