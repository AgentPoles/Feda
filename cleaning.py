...
from model import create_model, train_model, predict_next_word, tokenizer, predict_top_n_words_local_global_unique_words, train_local_model, train_global_model, evaluate_global_model
...
from decentralized import update_model_weights_from_contract
...
class TextClient(fl.client.NumPyClient):
    def __init__(self, model, tokenizer, train_data, val_data, username):
        self.model = model
    ...
    def update_data(self, train_data, val_data):
        self.train_data = train_data
        self.val_data = val_data
    ...
    def fit(self, parameters, config):
        ...
        self.model, self.tokenizer,a,b= train_global_model(self.train_data, self.val_data,self.tokenizer,username,self.model)
        print("fitted weights", len(self.model.get_weights()))
        return [np.array(w) for w in self.model.get_weights()], len(self.train_data), {}

    def evaluate(self, parameters, config):
        try:
            self.model.set_weights(parameters)
            loss, accuracy = evaluate_global_model(self.model,self.val_data,self.tokenizer, self.username)
...
def load_base_models(username):
    ...
    st.session_state['localtokenizer'] = tokenizer
    st.session_state['globaltokenizer'] = tokenizer
    save_user_local_model(username, model)
    save_user_global_model(username, model)
    local_tokenizer =  f"{username}_local_tokenizer.pickle"
    global_tokenizer =  f"{username}_global_tokenizer.pickle"
    save_tokenizer(tokenizer,"local_tokenizers", local_tokenizer)
    save_tokenizer(tokenizer,"global_tokenizers",global_tokenizer)
...
def load_user_models(username):
     ...
     st.session_state['localtokenizer'] = load_tokenizer("local_tokenizers", local_tokenizer)
     st.session_state['globaltokenizer'] = load_tokenizer("global_tokenizers", global_tokenizer)
...
def load_none_state():
    ...
    st.session_state['localtokenizer'] = None
    st.session_state['globaltokenizer'] = None
    st.session_state['isBase'] = True
...
with col_b:
        ...
        if st.button("🖥️"):
             ...
             if username is not None:
                 ...
                 if os.path.exists(filename):
                      ...
                      tokenizer = st.session_state['localtokenizer']
                      if model and tokenizer:
                           st.session_state['localmodel'],st.session_state['localtokenizer'],a,b = train_local_model(data,chat_history,tokenizer,username,model)
    ...
    with col2:
        if st.button("🌍"):
             ...
             if username is not None:
                 ...
                 if os.path.exists(filename):
                      data, chat_history = prepare_history_data_for_training(username)
                      model = st.session_state['globalmodel']
                      tokenizer = st.session_state['globaltokenizer']
                      if model and tokenizer:
                              client = TextClient(model, tokenizer, data, chat_history, username)
                              fl.client.start_numpy_client(
                                    server_address=SERVER_ADDRESS, client=client)
                              global_model_name = f"{username}_global_model.h5"
                              user_tokenizer_name = f"{username}_global_tokenizer.pickle"
                              st.session_state['globalmodel'] = load_model_from_file(global_model_name, "user_global_models")
                              st.session_state['globaltokenizer'] = load_tokenizer("global_tokenizers", user_tokenizer_name)
    ...
    with col3:
        if st.button("🧬"):
             username = st.session_state["user_name"]
             if username is not None:
                  globalmodel = st.session_state['globalmodel']
                  if globalmodel:
                       try: 
                            globalmodel = update_model_weights_from_contract(globalmodel)
                            if(globalmodel): 
                                 st.session_state['globalmodel'] = globalmodel
                                 save_user_global_model(username,globalmodel)
                       except Exception as e:
                            print("An error occurred while setting weights")
...
if 'localtokenizer' not in st.session_state:
    load_none_state()

if 'globaltokenizer' not in st.session_state:
    load_none_state()
                    ...
                    if st.session_state.get('localmodel') is None:
                        ...
                        try:
                            ...
                            predicted_words = predict_top_n_words_local_global_unique_words(
                                st.session_state['localmodel'],st.session_state['globalmodel'], st.session_state['localtokenizer'], st.session_state['globaltokenizer'], prompt)
